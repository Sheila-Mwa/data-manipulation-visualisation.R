...
head(clinical_data_long)
View(clinical_data_long)

# calculate the mean follow-up scores for each treatment group
...
clinical_data_histogram <- arranged_data %>% 
  ggplot(aes(age)) +
  geom_histogram(binwidth = 5, aes(group = age, fill = age)) +
...
# create a new column named score_change
clinical_data_new <- clinical_data %>%
  mutate(score_change = baseline_score - followup_score)  # Calculate individual score change

head(clinical_data_new)
View(clinical_data_new)

# calculating the mean using the new column
...
clinical_data_change <- clinical_data_new %>%
  ...
  labs(
    ...
    x = "Gender",
    y = "Score Change (Baseline - Follow-up)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold"),
...
head(clinical_new_variable)
View(clinical_new_variable)
...
baseline_score_relationship <- clinical_new_variable %>% 
  ggplot(aes(baseline_score, changed_score, color = changed_score)) +
  geom_point() +
  scale_color_gradient(low = "orange", high = "darkred") +
  labs(
    title = "Relationship between baseline score and changed_score",
    x = "baseline_score",
    y = "changed_score (followup - baseline)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(face = "bold"),
